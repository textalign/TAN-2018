...
<xsl:stylesheet xmlns="tag:textalign.net,2015:ns" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:function name="tan:expand-doc" as="document-node()*">
      ...
      <xsl:variable name="this-is-tan-lm" select="$this-tan-type = 'TAN-A-lm'"/>
            ...
            <xsl:choose>
               ...
               <xsl:when test="$this-class-number = 2">
                  ...
                  <xsl:variable name="dependencies-stripped-to-markers" as="document-node()*">
                     <xsl:choose>
                        <xsl:when
                           test="exists($adjustments-part-1) or exists($adjustments-part-2) or exists($these-ref-parents)">
         ...
         <xsl:choose>
            ...
            <xsl:when test="self::tan:vocabulary">
               <xsl:variable name="duplicate-vocab-item-names" as="element()*">
                  <xsl:for-each-group select="$target-resolved/tan:TAN-voc/tan:body//tan:item"
   ...
   <xsl:template match="/*[tan:head]" mode="core-expansion-terse-attributes">
      ...
      <xsl:copy>
         ...
         <xsl:apply-templates mode="#current">
            ...
            <xsl:with-param name="local-head" select="tan:head" tunnel="yes"/>
         </xsl:apply-templates>
      </xsl:copy>
   </xsl:template>
   ...
   <xsl:template match="*" mode="core-expansion-terse-attributes">
      ...
      <xsl:copy>
         ...
         <xsl:for-each select="$attributes-that-take-idrefs">
            ...
            <xsl:variable name="these-vals-normalized" select="tan:help-extracted($these-vals)"/>
            ...
            <xsl:if test="$diagnostics-on">
               ...
               <xsl:message
                  select="count($these-vals), 'normalized values: ', string-join($these-vals, ', ')"/>
               <xsl:message select="'these vals parsed: ', $these-vals-normalized"/>
               <xsl:message select="'this vocabulary: '"/>
               <xsl:message select="$this-vocabulary"/>
            </xsl:if>
            ...
            <xsl:if test="count($these-vals) = 0">
         ...
         <xsl:if test="exists(@href) and not((self::tan:location, self::tan:master-location))">
            <xsl:choose>
               ...
               <xsl:when test="doc-available($this-href)">
                  ...
                  <xsl:copy-of select="tan:error('tan08', $this-message, tan:copy-indentation($this-fix, .)/self::*, 'replace-self')"
                  />
               </xsl:when>
               <xsl:otherwise>
